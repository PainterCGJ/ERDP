...
extern "C" {
    ...
    int __cxa_guard_acquire(volatile int *guard_object) { return !*(volatile char *)guard_object; }

    void __cxa_guard_release(volatile int *guard_object) { *(volatile char *)guard_object = 1; }
     ...
     */
    void *malloc(size_t size) { return erdp_if_rtos_malloc(size); }

    /**
     * @brief 覆盖标准库的 free，使用 FreeRTOS 堆管理
     * @param ptr 要释放的内存指针
     */
    void free(void *ptr) { erdp_if_rtos_free(ptr); }
...
namespace erdp {
    ...
    std::function<void(Thread *)> Thread::killThreadHook = nullptr;

    Thread::Thread(void (*task_code)(void *p_arg), const char *name, uint32_t priority, size_t starck_size)
        : m_threadCode(task_code), m_stackSize(starck_size) {
        strcpy(m_name, name);
    }

    Thread::Thread(void (*task_code)(void *p_arg), void *p_arg, const char *name, uint32_t priority, size_t starck_size)
    ...
    Thread::Thread(std::function<void()> handle, const char *name, uint32_t priority, size_t stack_size)
    ...
    void Thread::kill(OS_TaskHandle handler) {
        if (killThreadHook) {
            killThreadHook(this);
        }
    ...
    void create_main_task() {
        Thread::m_mainTask =
            erdp_if_rtos_task_create(Thread::mainThread, "main", ERDP_CONFIG_MAIN_THREAD_STACK_SIZE, nullptr, 20);
    }
}    // namespace erdp
int main(void) {
...
void *operator new(size_t size) { return erdp_if_rtos_malloc(size); }

void *operator new[](size_t size) { return erdp_if_rtos_malloc(size); }

void operator delete(void *pointer) noexcept { erdp_if_rtos_free(pointer); }

void operator delete[](void *pointer) noexcept { erdp_if_rtos_free(pointer); }

void operator delete(void *pointer, size_t) noexcept { erdp_if_rtos_free(pointer); }
