...
uint32_t erdp_if_uart_get_PCLK(ERDP_Uart_t uart)
{
    ...
    {
    case ERDP_UART0:
        return (uint32_t)RCU_USART0;
    case ERDP_UART1:
        return (uint32_t)RCU_USART1;
    case ERDP_UART2:
        return (uint32_t)RCU_USART2;
    case ERDP_UART3:
        return (uint32_t)RCU_UART3;
    case ERDP_UART4:
        return (uint32_t)RCU_UART4;
    case ERDP_UART5:
        return (uint32_t)RCU_USART5;
    case ERDP_UART6:
        return (uint32_t)RCU_UART6;
    case ERDP_UART7:
        return (uint32_t)RCU_UART7;
    default:
        return 0; // Invalid UART
    }
}
...
void erdp_if_uart_send_bytes(ERDP_Uart_t uart, uint8_t *data, uint8_t len)
{
    uint32_t uart_base = uart_instance[uart];
